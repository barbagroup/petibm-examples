...
ax.loglog(dt_values, errors['ns']['linf'], label='$L_\infty$ (NS)',
          color='C0', marker='s')
ax.loglog(dt_values, errors['ib']['l2'], label='$L_2$ (IB)',
          color='black', linestyle=':', marker='x')
ax.loglog(dt_values, errors['ib']['linf'], label='$L_\infty$ (IB)',
          color='black', linestyle=':', marker='s')
# Add guide for second-order convergence.
markers = numpy.array([5e-4, 5e-2])
second = 5e-4 * (markers / markers[-1])**2
